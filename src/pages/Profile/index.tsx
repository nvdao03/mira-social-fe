...
function Profile() {
  const { id: currentUserId } = useContext(AppContext)
  ...
  const { data: followingsRes } = useQuery({
    queryKey: ['followings'],
    queryFn: () => followApi.getFollowings(currentUserId as string, queryConfig),
    keepPreviousData: true
  })

  const { data: followersRes } = useQuery({
    queryKey: ['followers'],
    queryFn: () => followApi.getFollowers(currentUserId as string, queryConfig),
    keepPreviousData: true
  })
  ...
  const handleFollow = useCallback(
    ...
    [followMutation, invalidateFollowQueries]
  )
  ...
  const handleUnfollow = useCallback(
    ...
    [unfollowMutation, invalidateFollowQueries]
  )

  const [isHover, setIsHover] = useState<boolean>(false)
  const [searchParams, setSearchParams] = useSearchParams()
  const defaultTab = searchParams.get('tab') || 'Posts'
  const [isActiveTab, setIsActiveTab] = useState<string>(defaultTab)

  // Check điều kiện nếu người ta đã follow mình
  const isFollowers = followersRes?.data?.data?.followers.some(
    (user: any) => user.user_followers._id === params.user_id
  )
  // Check điều kiện nếu mình đã follow người ta
  const isFollowing = followingsRes?.data?.data?.followings.some(
    (user: any) => user.user_followings._id === params.user_id
  )
  ...
  const profiles: ProfileType[] = profileRes?.data?.data || []
    ...
    <div className='relative pb-[45px] md:pb-[5px]'>
      {profiles.map((user) => (
        <div key={user._id}>
          ...
          {/* Verify warning */}
          {user.verify === 0 && user._id === currentUserId && (
              ...
              <div className={`mt-4 md:mt-8`}>
                ...
                <div className='mt-3 flex flex-col gap-[10px]'>
                  ...
                  <div className='flex items-end gap-1'>
                    ...
                    <span className='text-[15px] text-[#71767B]'>{new Date(user.createdAt).toLocaleDateString()}</span>
                  </div>
                </div>
