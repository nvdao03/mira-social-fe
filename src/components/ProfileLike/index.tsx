...
function ProfileLike() {
  ...
  const postsUserQuery = useInfiniteQuery({
    ...
    queryFn: ({ pageParam = queryConfig.page }) => {
      return userApi.getLikePostsProfile(params.user_id as string, { page: pageParam, limit: queryConfig.limit })
    },
  ...
  return <div className='mt-1 relative'>{renderContent()}</div>
}
